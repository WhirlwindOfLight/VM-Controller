...
MOUSE_REL_EVENTS = [REL_X, REL_Y]
MOUSE_ABS_EVENTS = [ABS_X, ABS_Y]
MOUSE_WHEEL_EVENTS = [REL_WHEEL, REL_WHEEL_HI_RES]
HI_RES_MULT = 120
...
class Mouse:
    def __init__(self, devName="vmController - Mouse"):
        ...
        self.absPos = [1, 1]
        self.virtAbsMouse = uinput.Device(MOUSE_BUTTONS + MOUSE_WHEEL_EVENTS + list(map(lambda x: x + ABS_RANGE, MOUSE_ABS_EVENTS)), devName + ABSMOUSE_SUFFIX)
        self.virtRelMouse = uinput.Device(MOUSE_BUTTONS + MOUSE_WHEEL_EVENTS + MOUSE_REL_EVENTS, devName + RELMOUSE_SUFFIX)
    ...
    def absToEvents(self, absMousePos, wheelMovement):
        ...
        if wheelMovement != 0:
            ...
            events.append(EventStruct(MOUSE_WHEEL_EVENTS[1], wheelMovement * HI_RES_MULT))
    ...
    def printEvents(self, events):
        ...
        for ev in events:
            print("[{}] - ({}, {})".format(mouseName, ev["event"], ev["value"]))
    ...
    def processEvents(self, events):
        ...
        for ev in events:
            myVirtMouse.emit(ev["event"], ev["value"], syn=False)
        myVirtMouse.syn()
